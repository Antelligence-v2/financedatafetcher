...
# Add data-fetch directory FIRST (before root) for warehouse/services imports
# This ensures data-fetch/src is found before root/src when both exist
data_fetch_dir = app_dir / "data-fetch"
# Remove if already in path, then add to front
if str(data_fetch_dir) in sys.path:
    sys.path.remove(str(data_fetch_dir))
sys.path.insert(0, str(data_fetch_dir))

# Add the app directory to Python path (where src/ folder is located)
# This comes after data-fetch so root src doesn't override data-fetch src
